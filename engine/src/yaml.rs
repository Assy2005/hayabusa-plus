...
impl ParseYaml {
    ...
    /// hayabusa-fx: walk a freshly-parsed YAML doc set and, for every
    /// `lookup:` array entry, load the referenced file into the global
    /// lookup-table registry. Tolerant of missing files / malformed
    /// entries — failures are surfaced via stderr but do not abort
    /// the rule load.
    fn register_lookup_tables(yaml_docs: &Vec<Yaml>, rule_path: &PathBuf) {
        use crate::detections::rule::lookup;
        for doc in yaml_docs {
            let Some(entries) = doc["lookup"].as_vec() else { continue };
            for entry in entries {
                let Some(name) = entry["name"].as_str() else { continue };
                let Some(file_str) = entry["file"].as_str() else { continue };
                // Resolve relative to the rule file's directory first,
                // then to the current working directory, then accept as-is.
                let mut tried = Vec::new();
                let rule_dir = rule_path.parent().map(|p| p.to_path_buf());
                let candidates: Vec<PathBuf> = [
                    rule_dir.as_ref().map(|d| d.join(file_str)),
                    Some(PathBuf::from(file_str)),
                ].into_iter().flatten().collect();
                let mut loaded = false;
                for cand in &candidates {
                    if cand.exists() {
                        match lookup::load_table_from_file(name, cand) {
                            Ok(n) => {
                                if std::env::var("HAYABUSA_FX_DEBUG").is_ok() {
                                    eprintln!("[hayfx-lookup] '{name}' <- {} ({n} entries)",
                                              cand.display());
                                }
                                loaded = true;
                                break;
                        ...
                        tried.push(format!("{}: not found", cand.display()));
                    }
                }
                if !loaded {
                    eprintln!("[hayfx-lookup] failed to load table '{name}': {}",
                              tried.join("; "));
                }
            }
        }
        ...
        if metadata.unwrap().file_type().is_file() {
            ...
            match YamlLoader::load_from_str(&read_content) {
                Ok(contents) => {
                    ...
                    // hayabusa-fx: pre-load any lookup tables referenced by this rule.
                    // We pass the rule's own directory as the base so authors can use
                    // relative paths like `file: ../lookups/loldrivers.txt`.
                    Self::register_lookup_tables(&contents, &path.as_ref().to_path_buf());
            ...
            yaml_docs = entries.try_fold(vec![], |mut ret, entry| {
                ...
                match YamlLoader::load_from_str(&read_content) {
                    Ok(contents) => {
                        Self::update_correlation_counts(self, &contents);
                        // hayabusa-fx: pre-load any lookup tables this rule
                        // references. Mirror of the same hook in the single-
                        // file load path above.
                        Self::register_lookup_tables(&contents, &entry.path());
