...
impl LeafMatcher for DefaultMatcher {
    ...
    fn is_match(&self, event_value: Option<&String>, recinfo: &EvtxRecordInfo) -> bool {
        ...
        let match_result = match pipe {
            ...
            // hayabusa-fx: lookup-table membership. Missing tables count
            // as no-match for `lookup:` (conservative) and full-match for
            // `not_lookup:` (analyst error doesn't silently hide events
            // that would otherwise have been flagged).
            PipeElement::Lookup(name) | PipeElement::NotLookup(name) => {
                let val = String::default();
                let event_value_str = event_value.unwrap_or(&val);
                let in_table = super::lookup::contains(name, event_value_str);
                let result = match pipe {
                    PipeElement::Lookup(_) => in_table.unwrap_or(false),
                    PipeElement::NotLookup(_) => !in_table.unwrap_or(true),
                    _ => unreachable!(),
                };
                Some(result)
            }
...
pub enum PipeElement {
    ...
    Utf16Be,
    Wide,
    // hayabusa-fx: lookup-table membership tests.
    // The String holds the table NAME (rule-author chosen), not a value.
    Lookup(String),
    NotLookup(String),
}
...
impl PipeElement {
    fn new(key: &str, pattern: &str, key_list: &Nested<String>) -> Result<PipeElement, String> {
        let pipe_element = match key {
            ...
            // hayabusa-fx: lookup-table modifiers. `pattern` is the table
            // name (a single string). Multi-name lists could be added by
            // accepting a Vec<String> here later.
            "lookup" => Some(PipeElement::Lookup(pattern.to_string())),
            "not_lookup" | "notlookup" => Some(PipeElement::NotLookup(pattern.to_string())),
            _ => None,
        };
