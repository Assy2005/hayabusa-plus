...
// hayabusa-fx: mimalloc deps removed; using system allocator. See Cargo.toml note.
use nested::Nested;
...
// hayabusa-fx: relying on system allocator instead of mimalloc.
...
Any hostnames added to the critical_systems.txt file will have all alerts above low increased by one level with a maximum of emergency level.";
        ...
        if stored_static.config.debug {
            ...
            println!();
            // hayabusa-fx: mi_stats_print_out removed (no mimalloc in this build).
        }
    }
