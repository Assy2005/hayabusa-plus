...
<details>
<summary><b>スキャンタブ</b> — 1 画面で対象選択から実行まで</summary>

```
┌─ ① 対象を選ぶ ─────────────┐  ┌─ ② スキャン設定 ───────────┐
│ 🖥️ このパソコンを検査       │  │ [標準][軽量][徹底][カスタム]│
│ ✓ 管理者権限あり           │  │                            │
│ ログ: 487 MiB              │  │ 最小レベル: medium ▼       │
│ [このパソコンをスキャン]   │  │ ☑ 同一検知を重複排除       │
│ [EVTX を取り込む]          │  │ ▶ 詳細オプション           │
│ [チャネル選択…]            │  │                            │
│                            │  │  ライブフィード            │
│ — または EVTX ファイルから─│  │  09:32:01 [CRIT] LSASS...│
│ [↓ ドロップ]               │  │  09:32:02 [HIGH] Sys 7045│
│ [test1.evtx 2.4 MiB] [SELECT]│  │  09:32:03 [MED ] Pwsh... │
└────────────────────────────┘  └────────────────────────────┘
┌─ ③ 実行 ────────────────────────────────────────────────────┐
│             [    スキャン開始   ]                            │
│   uploads/test1.evtx ・ 最小レベル: medium ・ 重複排除      │
└─────────────────────────────────────────────────────────────┘
```
...
<details>
<summary><b>ダッシュボード</b> — KPI + 振舞い異常 + 統計チャート</summary>

```
┌── KPI ────────────────────────────────────────────────────┐
│  544        9          3              15                 │
│  検知総数   Crit+High  影響ホスト数   発火ルール種数      │
└───────────────────────────────────────────────────────────┘

┌─ 重要度内訳 ──┐  ┌─ 時系列 ──────────────────────────────┐
│   ⭕ 544     │  │  ▁▁▁▂▂▃▃▂▆▆▇▆▅▄▃▂▂▁ (1時間バケット)   │
│  Detections  │  │  [crit][high][medium][low]            │
└──────────────┘  └───────────────────────────────────────┘

┌─ 検知パターン異常 (Sigma では書けない振舞い) ────────────┐
│ 種別   重要 対象               件数/倍率   説明         │
│ バースト HI Potentially Mal PwSh 691件×2492 平常の2492倍 │
│ 拡散    CR LSASS Access...     3ホスト/8件 同ルール3host │
│ ...                                                    │
│ [クリックで該当検知のハント検索へ →]                   │
└─────────────────────────────────────────────────────────┘
```
...
<details>
<summary><b>検知詳細パネル</b> — サブタブで深掘り</summary>

```
🔴 critical  Suspicious LSASS Access via comsvcs.dll
#job1234 / line 12
[attack.credential_access] [T1003.001 · LSASS メモリ]

[判定: TP] [FP] [未]   [抑制: host×rule] [rule] [host]

┌─ [概要][ルール][関連検知 (3)][同ルール履歴 (5)][プロセスツリー][Raw JSON] ─┐
│                                                                          │
│ なにを検知している?              重要度 — 重大 (critical)                │
│ comsvcs.dll の MiniDump 経由で  実害が出ている / 出る寸前。             │
│ LSASS をダンプする手法。        即時調査と封じ込めが必要。              │
│ Cobalt Strike / Mimikatz 等で                                           │
│ 定番。                          [対応目安: 今すぐ確認]                  │
│                                                                          │
│ 誤検知の例                       次にすべきこと                          │
│ • 検体解析環境での意図的実行    1. lsass.exe を触ったプロセスの**親系統**│
│                                    を Sysmon EID 1 / Security 4688 で確認│
│                                  2. RDP / WMI / PsExec 経由で起動された  │
│                                    なら、横展開のシグナル               │
└──────────────────────────────────────────────────────────────────────────┘
```
...
<details>
<summary><b>ハントタブ</b> — 仮説駆動の検索 + ピボット</summary>

```
┌── 条件 ─────────────┐  ┌── 結果 ──────────────────────────┐
│ クイックプリセット   │  │  14,244 件 (host=LAPTOP-*)        │
│ [直近24hの重大]      │  │                                   │
│ [痕跡隠蔽行為]      │  │  [一覧][ルール別][ホスト別]      │
│ [認証情報窃取]      │  │  [時系列][重要度別]              │
│ [永続化]            │  │                                   │
│ [横展開兆候]        │  │  時刻      Lvl  ルール  ホスト   │
│ [最多発火]          │  │  09:42:36  HI   Susp... LAPTOP   │
│                     │  │  09:31:46  HI   Susp... LAPTOP   │
│ ホスト: WS-*        │  │  ...                              │
│ 時間: 直近24h ▼     │  │                                   │
│ レベル: 🔴🟠🟡     │  │                                   │
│ チャネル: System ▼  │  │                                   │
