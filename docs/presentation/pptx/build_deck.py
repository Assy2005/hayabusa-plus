...
def slide_dev_gui(prs, n, total):
    """10: 開発③ GUI — 結果を「読める」にする画面をゼロから開発."""
    s = add_slide(prs, BG_LIGHT)
...
def slide_dev_anomaly(prs, n, total):
    """11: 開発③+ — ルールでは書けない「いつもと違う」も自作の分析で検出."""
    s = add_slide(prs, BG_LIGHT)
...
def main():
    ...
    slide_dev_gui(prs, 10, total)       # 10 — 開発③ GUI
    slide_dev_anomaly(prs, 11, total)   # 11 — 開発+α 異常検知
    slide_demo(prs, 12, total)          # 12 — デモ
    slide_summary(prs, 13, total)       # 13 — まとめ
    slide_thanks(prs, total, total)     # 14 — おわり
