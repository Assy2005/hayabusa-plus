...
SPEAKER_NOTES = {
    ...
    13: (
        "研究として大事な「で、効果は?」にお答えします。\n"
        "まず核心の自作 lookup 拡張です。既知の悪用ドライバのハッシュを仕込んだ検証用"
        "ログを読み込ませると、critical で検知できました。ルール本文は 1 行も変えず、"
        "外部のリスト側だけで検知できる ── この仕組みが狙いどおり動くことを確認できた、"
        "ということです。\n"
        "次に、公開されている攻撃サンプル 6 種で試すと、5 種で脅威を検知できました。"
        "ひとつ正直にお伝えすると、ここで鳴っているのは上流ルールと自作分の合算で、"
        "自作 13 ルールは対応する痕跡が出たときに鳴る設計です。ですので「すべてを自作"
        "ルールで捕まえた」とは申しません。"
    ),
    15: (
