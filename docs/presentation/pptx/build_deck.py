...
def slide_evaluation(prs, n, total):
    ...
    add_text(s,
             "※ ②で鳴ったのは既存ルールと自作分の合わせ技。自作の 13 本は、"
             "対応する痕跡（wevtutil 等）が出たログで鳴る作りです。",
             x=7.15, y=5.62, w=5.45, h=0.75, size=10, color=MUTED,
             line_spacing=1.3)

    add_text(s, "実際のログを読ませて、危ない動きがちゃんと “検知” として浮かび上がるか確かめました。",
...
SPEAKER_NOTES = {
    ...
    13: (
        "ここは「で、結局ちゃんと効くの?」に答えるスライドです。\n"
        "まず自作した lookup 拡張から。悪用ドライバの“指紋”をわざと仕込んだログを"
        "読ませてみたら、ちゃんと critical で引っかかりました。ルールは 1 行も書き換えず、"
        "リスト側を差し替えるだけ。狙いどおり動きました。\n"
        "次に、公開されている攻撃サンプル 6 種で試したら、5 種で検知できました。"
        "ここだけ正直に言うと、鳴っているのは既存ルールと自作分の合わせ技で、自作の"
        "13 本は“その痕跡が出たとき”に鳴る作りです。なので「全部うちのルールで捕まえた」"
        "とまでは言いません。"
    ),
    15: (
